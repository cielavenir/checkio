...
def solver(_data,_words):
	...
	for i in range(len(flags)):
		for j in range(len(flags[i])):
			if flags[i][j]>=0:
				if flags[i][j]&VERTICAL==0 and i<len(flags)-1 and flags[i+1][j]>=0:
					k=0
					while i+k<len(flags) and flags[i+k][j]>=0:
						flags[i+k][j]|=VERTICAL
						k+=1
					_lst.append((i,j,k,VERTICAL))
				if flags[i][j]&HORIZONTAL==0 and j<len(flags[i])-1 and flags[i][j+1]>=0:
					k=0
					while j+k<len(data[i]) and flags[i][j+k]>=0:
						flags[i][j+k]|=HORIZONTAL
						k+=1
					_lst.append((i,j,k,HORIZONTAL))
	...
	while _lst:
		...
		while i>=0:
			if any(
				_lst[i][3]==HORIZONTAL and _i<=_lst[i][0]<_i+k and _lst[i][1]<=j<_lst[i][1]+_lst[i][2]
				if z==VERTICAL else
				_lst[i][3]==VERTICAL and j<=_lst[i][1]<j+k and _lst[i][0]<=_i<_lst[i][0]+_lst[i][2]
				for _i,j,k,z in lst
			):
